    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <KpiWidget title="Gross Profit Margin" value={`${grossMargin.toFixed(1)}%`} change="Revenue - COGS" icon={PieChartIcon} iconColor="text-emerald-400" />
          <KpiWidget title="Operating Margin" value={`${operatingMargin.toFixed(1)}%`} change="Profitability from operations" icon={ScaleIcon} iconColor={operatingMargin > 0 ? "text-emerald-400" : "text-red-400"} />
          <KpiWidget title="Net Profit Margin" value={`${netProfitMargin.toFixed(1)}%`} change="After all expenses & taxes" icon={TrendingUpIcon} iconColor={netProfitMargin > 0 ? "text-emerald-400" : "text-red-400"}/>
          <KpiWidget title="Net Income" value={fmtEuro(netIncome)} change="The bottom line" icon={TrendingUpIcon} iconColor={netIncome > 0 ? "text-emerald-400" : "text-red-400"}/>
        </div>
      </section>
